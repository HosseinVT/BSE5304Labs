...
BasinData[is.na(BasinData)]=0

#### Forecasting Data
library(lattice)

gfsvars=grepVar('precip', service = 'gfs', complete = TRUE)
precipvars <- getPoint(c(myflowgage$declon, myflowgage$declat), vars = c('Total_precipitation_surface_Mixed_intervals_Accumulation'), 
                       service ="gfs",day = today-1)
GFSPrecip=aggregate(precipvars,as.Date(time(precipvars)),sum)
plot(GFSPrecip)


# Temperature
gfsvars=grepVar('temp', service = 'gfs', complete = TRUE)
tempvars <- getPoint(c(myflowgage$declon, myflowgage$declat), vars = c('Temperature_surface'), 
                     service ="gfs",day = today-1)
temp=aggregate(tempvars,as.Date(time(tempvars)),sum)
plot(temp)

GFSMaxTemp=aggregate(tempvars,as.Date(time(tempvars)),max)
GFSMinTemp=aggregate(tempvars,as.Date(time(tempvars)),min)

plot(GFSMinTemp, ylim=c(275,300))
lines(GFSMaxTemp)

#Add forecast data to WXData
P=GFSPrecip/10
MaxTemp=GFSMaxTemp-273
MinTemp=GFSMinTemp-273

df=merge(P, MaxTemp, MinTemp, all = TRUE)
...
#mdat=data.frame(date = modeldata$date, Qm3ps=modeldata$Qmm,
#P=modeldata$P,MaxTemp=modeldata$MaxTemp,MinTemp=modeldata$MinTemp)
BasinData=full_join(BasinData,dff,by=c('date','P','MaxTemp','MinTemp'))
...
plot(TMWB$date,TMWB$SNO,xlab='Date',ylab='Snow (mm)')
#
...
plot(TMWB$date,TMWB$PET,xlab='Date',ylab='PET (mm)')
...
plot(BasinTMWB_JO$date,BasinTMWB_JO$Qpred,xlab='Date',ylab='Predicted Stremflow (m3/s)')
...
sed=TIC05$sed+TIC04$sed+TIC03$sed+TIC02$sed+TIC01$sed
sedT=data_frame(date=TIC05$date,sedi=sed)
plot(sedT$date,sedT$sedi, type="l",ylim=c(1,30), xlab="Date" , ylab = "Sedimenat (kg.ha^-1)")


################################################################################

####### Dissolved P Loss From Fertilized Areas
...
####### P Loss From Plant-Soil Complex
...
################################################################################
##########################   Calculate Total P Loss    #########################
################################################################################
#Dissolved P Loss From Fertilized Areas + P Loss From Plant-Soil Complex 

TPLoss <- function(RunoffModel=TIC05, tau=9.3, dt=1, kF=0.015, TI=1, Tavg=Tavg){
...
###################################### run function for all TI classes ################
DPTI05 = TPLoss (RunoffModel=TIC05, tau=9.3, dt=1, kF=0.015, TI=5, Tavg=Tavg)
DPTI04 = TPLoss (RunoffModel=TIC04, tau=9.3, dt=1, kF=0.015, TI=4, Tavg=Tavg)
DPTI03 = TPLoss (RunoffModel=TIC03, tau=9.3, dt=1, kF=0.015, TI=3, Tavg=Tavg)
DPTI02 = TPLoss (RunoffModel=TIC02, tau=9.3, dt=1, kF=0.015, TI=2, Tavg=Tavg)
DPTI01 = TPLoss (RunoffModel=TIC01, tau=9.3, dt=1, kF=0.015, TI=1, Tavg=Tavg)

PT=DPTI05$DS+DPTI04$DS+DPTI03$DS+DPTI02$DS+DPTI01$DS+DPTI05$
  POT=data_frame(date=DPTI05$date,PP=PT)
plot(POT$date,POT$PP, type="l", xlab="Date" , ylab = "P Loss (kg.ha^-1)")
