...
# 
setwd(mygitdir)
system(paste0("git config --global user.email 'hosseinahmadi@vt.edu'")) 
...
# Make a note of why mean VS median for the method section and get
# it stuck into the article 
...
# Try out median prior or during sensitivity analysis for these parameter
...
######### Calculation of sediment for each HRU #################################
...
TotalP = sum(TIC05$area*(DPTI05$DF + DPTI05$DS)+
              TIC04$area*(DPTI04$DF + DPTI04$DS)+
              TIC03$area*(DPTI03$DF + DPTI03$DS)+
              TIC02$area*(DPTI02$DF + DPTI02$DS)+
              TIC01$area*(DPTI01$DF + DPTI01$DS))

Years = Removal_amount/TotalP*(2022-2016)
Years
...
points(DPLT_2$date,DPLT_2$LT,col="blue")

dLT= sum(DPLT_2$LT)-sum(DPLT$LT)
dLT
...
plot(DPLT$date,DPLT$LT, type="l",ylim=c(1,5),
     xlab = "Date", ylab ="P", 
     main = "Average Watershed P Export") +
points(DPLT_3$date,DPLT_3$LT,col="red")

dLT2= sum(DPLT_3$LT)-sum(DPLT$LT)
dLT2
